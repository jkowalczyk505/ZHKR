...
function BreedingMap() {
          ...
          data.map(async (item) => {
            ...
            try {
              const r = await fetch(
                ...
                {
                  headers: { "User-Agent": "ZHKR-website/1.0 (+https://kawierasowe.pl)" }
                }
              );
  ...
  return (
    <div className="map-wrapper">
      ...
      <MapContainer center={center} zoom={zoom} scrollWheelZoom={false} className={`map-container ${loading ? "loading" : ""}`}>
        <TileLayer
        ...
        {!loading &&
          ...
          >
            {markers.map((b) => (
              <FlyToMarker key={b.numer} position={b.coords} icon={CustomMarker}>
                <strong>{b.nazwa}</strong><br />
                Rasy: {b.rasy}
              </FlyToMarker>
            ))}
          </MarkerClusterGroup>}
